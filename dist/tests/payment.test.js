...
describe("payment", () => {
    ...
    before(async () => {
        ...
        const airdropSignature = await provider.connection.requestAirdrop(payerKeypair.publicKey, 10 * LAMPORTS_PER_SOL);
        await provider.connection.confirmTransaction(airdropSignature);
        // Create a new mint
        mint = await spl.createMint(provider.connection, payerKeypair, payerKeypair.publicKey, null, 9);
        // Create a token account for the user
        userTokenAccount = await spl
            .getOrCreateAssociatedTokenAccount(provider.connection, payerKeypair, mint, payerKeypair.publicKey)
            .then((account) => account.address);
        // Mint some tokens to the user
        await spl.mintTo(provider.connection, payerKeypair, mint, userTokenAccount, payerKeypair.publicKey, 1000000000000 // 1000 tokens
        );
        // Derive the payment state account PDA
        [paymentStatePDA] = PublicKey.findProgramAddressSync([Buffer.from("payment-state")], program.programId);
        // Derive the program token account PDA
        [programTokenPDA] = PublicKey.findProgramAddressSync([Buffer.from("program-token"), mint.toBuffer()], program.programId);
        [programSolAccount] = PublicKey.findProgramAddressSync([Buffer.from("program-token"), spl.NATIVE_MINT.toBuffer()], program.programId);
    ...
    it("Initializes the payment state", async () => {
        ...
        assert.strictEqual(paymentStateAccount.owner.toBase58(), payerKeypair.publicKey.toBase58(), "Owner public key doesn't match");
        assert.strictEqual(paymentStateAccount.enabled, true, "Payment state should be enabled");
        assert.deepStrictEqual(paymentStateAccount.feeToAccount, Array(32).fill(PAYMENT_STATE_ACCOUNT_FILL), "Fee to account doesn't match");
    });
    it("Initializes the program token", async () => {
        ...
        const [expectedProgramToken] = PublicKey.findProgramAddressSync([Buffer.from("program-token"), mint.toBuffer()], program.programId);
        assert(programTokenPDA.equals(expectedProgramToken), "SPL Program token address is not correctly derived");
        const [expectedProgramSolAccount] = PublicKey.findProgramAddressSync([Buffer.from("program-token"), spl.NATIVE_MINT.toBuffer()], program.programId);
    ...
    it("Deposits SOL", async () => {
        ...
        const { userAccountPDA, userSolAccountInfo, userBalanceAfter, programBalanceAfter, } = await depositSol(provider, program, payerKeypair, account, sn, amount, frozen, expiredAt);
        ...
        assert.strictEqual(programBalanceAfter.toString(), expectedBalance.toString(), `Program SOL balance is incorrect. Expected ${expectedBalance}, got ${programBalanceAfter}`);
    });
    /*
